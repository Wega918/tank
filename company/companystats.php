...
$res_company_sum_exp = mysqli_query($mysqli,'SELECT sum(company_exp_stats) FROM company_user WHERE `company` = "'.$company['id'].'" ');
if (FALSE === $res_company_sum_exp) die("Select sum failed: ".mysqli_error);
$row = mysqli_fetch_row($res_company_sum_exp);
$sum_exp = $row[0];
...
Дата создания: <span class="green2" w:id="creationTime">'.vremja($company['time']).'</span><br>
Возраст (дней): <span class="green2" w:id="clanAgeDays">'.floor((time()-$company['time'])/86400).'</span><br>
Время обнуления опыта: <span class="green2" w:id="resetMinorExpTime">'.vremja($company['time_obnul_exp']).'</span><br>
Всего опыта после обнуления: <img class="price_img_" src="/images/icons/exp.png" alt="опыт" title="опыт"> <span class="green2" w:id="minorExpSum">'.ceil($sum_exp).'</span>
</div>
</div>
...
//$res_reit_us = $mysqli->query('SELECT * FROM `company_user` WHERE `company` = '.$company['id'].' ORDER BY `company_exp_stats` desc LIMIT '.$start.','.$max.' ');
...
<a class="white" w:id="profileLink" href="/profile/'.$us_['id'].'"><img class="vb" height="14" width="14" src="/images/side/'.$side.'/'.$traning['rang'].''.$viz.'.png?1"> '.$us_['login'].', '.$company_rang.', <img class="ico vm" src="/images/icons/exp.png" alt="опыт" title="опыт"> '.n_f($us['company_exp_stats']).'</a>
</td></tr>';
}
...
$mysqli->query('UPDATE `company_user` SET `company_exp_stats` = "0" WHERE `company` = '.$company['id'].'');
$mysqli->query('UPDATE `company` SET `time_obnul_exp` = "'.time().'" WHERE `id` = '.$company['id'].'');
header('Location: ?');
exit();
}
