...
echo '<table><tbody><tr>
<td style="width:50%;padding:0 3px;"><a class="simple-but blue" href="/rating/bz_eggs/"><span><span>Пасхальные яйца</span></span></a></td>
<td style="width:50%;padding:0 3px;"><a class="simple-but gray" href="/rating/bz_miss/"><span><span>Выполненные задания</span></span></a></td>
</tr></tbody></table>';



$res = $mysqli->query('SELECT * FROM `bz_user` WHERE `id` ORDER BY `miss` desc LIMIT 100000');
...
<td class="va_m nwr p5 ta_r"><img class="vb" height="14" width="20" src="/images/icons/exp.png"> '.$usr['miss'].'</td>
</tr></tbody></table>
...
echo '<div class="white medium cntr bold mb2">Лучшие по выполненым заданиям</div>';
...
$start = $max*$page-$max;
$k_post[0] = $start+1;
$res = $mysqli->query('SELECT * FROM `bz_user` WHERE `id` ORDER BY `miss` desc LIMIT '.$start.','.$max.' ');
while ($usr = $res->fetch_array()){
$reyt = ''.$k_post[0]++.'';
...
if ($k_page > 1) {
echo str('/rating/bz_miss/?',$k_page,$page); // Вывод страниц
}
