...
$res1 = $mysqli->query('SELECT * FROM `dm_year` WHERE `time_end` != "0" ORDER BY `time_end` DESC LIMIT 4');
 ...
 */
$res = $mysqli->query('SELECT * FROM `dm_year_results` WHERE `user` = "'.$user['id'].'" and `dm_year_id` = "'.$p_end['dm_year_id'].'" ');
...
$res2 = $mysqli->query('SELECT * FROM `dm_year_results` WHERE `dm_year_id` = "'.$p_end['dm_year_id'].'" and `kill` > "0" ORDER BY `kill` DESC LIMIT 3');
...
$res3 = $mysqli->query('SELECT * FROM `dm_year_results` WHERE `dm_year_id` = "'.$p_end['dm_year_id'].'" and `uron` > "0" ORDER BY `uron` DESC LIMIT 3');
...
echo '<a w:id="backToPve" class="simple-but gray mt10" href="/dm_year/"><span><span>Назад к сражениям</span></span></a>';
