...
$p1 = $res->fetch_assoc();
if($p1){
$res = $mysqli->query('SELECT * FROM `pve` WHERE `id` ORDER BY `time_pve` desc LIMIT 1');
$p = $res->fetch_assoc();
}else{
...
$resb_s = $mysqli->query('SELECT * FROM `boevaya_sila` WHERE `user` = "'.$user['id'].'" and `local` = "2" limit 1');
$b_s = $resb_s->fetch_assoc();


if($p['time']<=(time()+1) and $p['time_pve']==0){
$mysqli->query('UPDATE `pve` SET `time_pve` = '.(time()+310).' WHERE `id` = '.$p['id'].' LIMIT 1');
$mysqli->query('DELETE FROM `pve_results` WHERE `pve_id` = "'.$p['pve_id'].'" and `uron` != "0" ');
}

if($p['time_pve']>=time() && $p['time_pve']<=(time()+310) and $p['time_pve']>=(time()+300)){
echo '<div class="buy-place-block"><div class="feedbackPanelINFO"><div class="line1"><span class="feedbackPanelINFO">До начала боя: '.tls($p['time_pve']-(time()+300)).' секунд</span></div></div></div>';
}

$res = $mysqli->query('SELECT * FROM `pve_user` WHERE `user` = '.$user['id'].' and `pve_id` = '.$p['pve_id'].' ');
$p_u = $res->fetch_assoc();


####################################################################################################################
if($p['time_pve']>=time() && $p['time_pve']<=(time()+300) and $p_u){
$pvp_us = $mysqli->query('SELECT * FROM `pve_user` WHERE `pve_id` = "'.$p['pve_id'].'" ORDER BY `id` desc');
...
$nick = $pvu['nick'];$pve_id = $p['pve_id'];$hp = 0;$bot = 1;$stmt_g->execute();$res = $stmt_g->get_result();$goal = $res->fetch_assoc(); 
### UPDATE
$stmt = $mysqli->prepare ("UPDATE `pve_user` SET `goal` =  ? WHERE `id` = ? LIMIT 1");
$stmt->bind_param("ss", $goal, $id);$goal = $goal['id'];$id = $pvu['id'];$stmt->execute();
}
if($pvu['bot']==1){
$mysqli->query('UPDATE `pve_user` SET `time_attack` = "'.rand(5,10).'" WHERE `id` = '.$pvu['id'].' ');
}
}
header('Location: /pve/battle/');
exit();
}
####################################################################################################################
...
for ($i = 1; $i<9; $i++){
$res = $mysqli->query('SELECT * FROM `pve` WHERE `pve_id` = '.$i.' ');
$p = $res->fetch_assoc();
if($i==1){$nameBattle = 'Битва на Халхин-Голе';$tipBattle = 1;$date =  strtotime('08:00');}//08:00
if($i==2){$nameBattle = 'Битва за о. Сахалин';$tipBattle = 2;$date =  strtotime('10:00');}//10^00
if($i==3){$nameBattle = 'Блокада Ленинграда';$tipBattle = 1;$date =  strtotime('12:00');}//12^00
if($i==4){$nameBattle = 'Битва за Харьков';$tipBattle = 2;$date =  strtotime('14:00');}//14^00
if($i==5){$nameBattle = 'Битва за Сталинград';$tipBattle = 1;$date =  strtotime('16:00');}//16^00
if($i==6){$nameBattle = 'Битва за Днепр';$tipBattle = 2;$date =  strtotime('18:00');}//18^00
if($i==7){$nameBattle = 'Битва за Москву';$tipBattle = 1;$date =  strtotime('20:00');}//20^00
if($i==8){$nameBattle = 'Курская дуга';$tipBattle = 2;$date =  strtotime('23:00');}//23:00
if($date<=time()){$dateStart = ($date+86400);}else{$dateStart = ($date);}
if(!$p){
$mysqli->query('INSERT INTO `pve` SET `time` = '.$dateStart.' , `pve_id` = '.$i.' , `tip` = '.$tipBattle.' , `name` = "'.$nameBattle.'" ');
}elseif($p and $p['time']!=$dateStart){
$mysqli->query('UPDATE `pve` SET `time` = "'.$dateStart.'" WHERE `id` = '.$i.' LIMIT 1');
}elseif($p and $p['tip']!=$tipBattle){
$mysqli->query('UPDATE `pve` SET `tip` = "'.$tipBattle.'" WHERE `id` = '.$i.' LIMIT 1');
}elseif($p and $p['name']!=$nameBattle){
$mysqli->query('UPDATE `pve` SET `name` = "'.$nameBattle.'" WHERE `id` = '.$i.' LIMIT 1');
}elseif($p and $p['pve_id']!=$i){
$mysqli->query('UPDATE `pve` SET `pve_id` = "'.$i.'" WHERE `id` = '.$i.' LIMIT 1');
}
}
...
$res = $mysqli->query('SELECT * FROM `pve` WHERE `time_end` != "0" ORDER BY `time_end` desc LIMIT 1');
$p_end = $res->fetch_assoc();

if($p_end['time_end']>=(time()-30)){
...
<div class="mb5 inbl"><div class="thumb fl"><img src="/images/battles/batt'.$p_end['pve_id'].'.jpg" style="width:100%; border-radius: 9px;"><span class="mask2">&nbsp;</span></div>
...
$res = $mysqli->query('SELECT * FROM `pve_results` WHERE `user` = "'.$user['id'].'" and `pve_id` = "'.$p_end['pve_id'].'" LIMIT 1');
...
$res = $mysqli->query('SELECT * FROM `pve_results` WHERE `pve_id` = "'.$p_end['pve_id'].'" and `bot` = "0" and `kill` > "0" ORDER BY `kill` DESC LIMIT 3');
while ($top_kill = $res->fetch_array()){
$res1 = $mysqli->query('SELECT * FROM `users` WHERE `id` = "'.$top_kill['user'].'" ');
$us = $res1->fetch_assoc();
...
echo '<a href="/profile/'.$top_kill['user'].'/"><img class="vb" height="14" width="14" src="/images/side/'.$side.'/'.$traning['rang'].''.$viz.'.png?1"> <span class="yellow1">'.$top_kill['nick'].'</span></a>: '.$top_kill['kill'].' врагов<br>';
}

echo '<br><span class="green2">Лучшие по урону</span><br>';
$res = $mysqli->query('SELECT * FROM `pve_results` WHERE `pve_id` = "'.$p_end['pve_id'].'" and `bot` = "0" and `uron` > "0" ORDER BY `uron` DESC LIMIT 3');
...
if($us['side'] == 1){$side = 'federation';}else{$side = 'empire';}
if($us['viz'] > (time()-$sql['online'])){$viz = '';}else{$viz = '_off';}
echo '<a href="/profile/'.$top_uron['user'].'/"><img class="vb" height="14" width="14" src="/images/side/'.$side.'/'.$traning['rang'].''.$viz.'.png?1"> <span class="yellow1">'.$top_uron['nick'].'</span></a>: '.$top_uron['uron'].' урона<br>';
}

echo '</div><br>
В битве сражались:<br>
Танки: <span class="green1">'.$p_end['where_user'].'</span> | Враги: <span class="red1">'.$p_end['where_ank'].'</span><br>
Выжили:<br>
Танки: <span class="green1">'.$p_end['survived_user'].'</span> | Враги: <span class="red1">'.$p_end['survived_ank'].'</span><br>
</div></div></div></div></div></div></div></div></div></div></div>';

}else{

if($p['time']<(time()+(300))){$_SESSION['pve']=777;}else{$_SESSION['pve']=0;}
if($p['tip']==1){$tipB = 'Оборонительная операция';}else{$tipB = 'Наступательная операция';}


if(time()-$p['time_pve']>(600)){
$mysqli->query('UPDATE `pve` SET `time_pve` = "0" WHERE `id` = '.$p['id'].' ');
}
...
$col_u_ = $mysqli->query("SELECT COUNT(*) FROM `pve_user` WHERE `pve_id` = ".$p['pve_id']." and `user` != '0' ");
$col_us = $col_u_->fetch_array(MYSQLI_NUM);

//<center><img src="/images/war'.$p['id'].'.png" style="width:99%;"></center>
...
echo '<div class="white medium cntr bold mb5">Ближайшее сражение</div>';

echo '<div class="mb5 inbl">
<div class="thumb fl"><img src="/images/battles/batt'.$p['id'].'.jpg" style="width:100%; border-radius: 9px;"><span class="mask2">&nbsp;</span></div>
<div class="ml58 small white sh_b bold"><span class="green2">'.$p['name'].'</span><br>';
if($p1){
if($p1['time_pve']>=time() && $p1['time_pve']>=(time()+301)){
echo '<span><span>до начала боя '.tls($p1['time_pve']-(time()+300)).' секунд ('.$col_us[0].' заявок)</span></span>';
}
if($p1['time_pve']>=time() && $p1['time_pve']<=(time()+300)){
if((300-($p1['time_pve']-time()))  >= 60  ){
$m = floor((300-($p1['time_pve']-time()))/60);
$s = (floor((300-($p1['time_pve']-time())))-(floor((300-($p1['time_pve']-time()))/60)*60));
if((floor((300-($p1['time_pve']-time())))-(floor((300-($p1['time_pve']-time()))/60)*60))<10){$ss = 0;}
$time = '0'.$m.':'.$ss.''.(floor((300-($p1['time_pve']-time())))-(floor((300-($p1['time_pve']-time()))/60)*60)).'';
}else{
$s = (floor((300-($p1['time_pve']-time())))-(floor((300-($p1['time_pve']-time()))/60)*60));
if((floor((300-($p1['time_pve']-time())))-(floor((300-($p1['time_pve']-time()))/60)*60))<10){$ss = 0;}
$time = '00:'.$ss.''.(floor((300-($p1['time_pve']-time())))-(floor((300-($p1['time_pve']-time()))/60)*60)).'';
}
...
echo '<br><span class="gray1">'.$tipB.'</span></div>
...
if(!$p_u and $p['time_pve']==0){
/* if($p['time']>(time()+(1800))){
echo '<div class="white medium cntr bold mb5">Заявку можно подать за 30 минут до начала</div>';
}else{
echo '<div class="bot"><a class="simple-but border" href="?start"><span><span>Взвод, подъем! В атаку!</span></span></a><div style="position:relative;"><span class="digit2 esmall"><span class="l">&nbsp;</span><span class="m">+</span><span class="r">&nbsp;</span></span></div></div>';
} */

echo '<div class="bot"><a class="simple-but border" href="?start"><span><span>Взвод, подъем! В атаку!</span></span></a><div style="position:relative;"><span class="digit2 esmall"><span class="l">&nbsp;</span><span class="m">+</span><span class="r">&nbsp;</span></span></div></div>';

}elseif($p_u['id']){
echo '<div class="white medium cntr bold mb5">Вы в рядах участников</div>';
}elseif($p['time_pve']!=0 and !$p_u){
echo '<div class="white medium cntr bold mb5">Вы не успели на это сражение</div>';
}
...
if($p['time_pve']!=0){header('Location: ?');exit();}
...
$stmt_g = $mysqli->prepare('SELECT t.id FROM pve_user as t,
			...
			LIMIT 1');
$stmt_g->bind_param("sss", $nick, $pve_id, $hp);
$nick = $user['login'];$pve_id = $p['pve_id'];$hp = 0;$stmt_g->execute();$res = $stmt_g->get_result();$goal = $res->fetch_assoc();
if(!$goal['id']){$goal['id'] = 0;} 
...
`a` = '.($u_t['a']).', `b` = '.($u_t['b']).', `t` = '.($u_t['t']).', `p` = '.($u_t['p']*2).', `p_` = '.($u_t['p']*2).', 
`pve_id` = '.$p['pve_id'].', `user` = '.$user['id'].' ');
$mysqli->query('INSERT INTO `pve_results` SET `exp` = "10", `silver` = "10", `user` = "'.$user['id'].'", `nick` = "'.$user['login'].'", `pve_id` = "'.$p['pve_id'].'", `time` = "'.time().'" ');
...
echo '<a w:id="refresh" href="?" class="simple-but gray"><span><span>Обновить</span></span></a>';
...
if(!$b_s){
$mysqli->query('INSERT INTO `boevaya_sila` SET `user` = '.$user['id'].', `local` = "2", `bon_col` = "1", `tip` = "1" ');
}else{
$mysqli->query("UPDATE `boevaya_sila` SET `local` = '2', `bon_col` = '1', `tip` = '1' WHERE `id` = '".$b_s['id']."' LIMIT 1");
}
if($p_u['id']){
$res = $mysqli->query('SELECT * FROM `users_tanks` WHERE `user` = '.$user['id'].' and `active`  = "1" LIMIT 1');
$u_t = $res->fetch_assoc();
$param = 50;
$u_t['a'] = ($u_t['a']+$param);$u_t['b'] = ($u_t['b']+$param);$u_t['t'] = ($u_t['t']+$param);$u_t['p'] = ($u_t['p']+$param);
$mysqli->query('UPDATE `pve_user` SET `a` = '.($u_t['a']).', `b` = '.($u_t['b']).', `t` = '.($u_t['t']).', `p` = '.($u_t['p']*2).', `p_` = '.($u_t['p']*2).' WHERE `id` = '.$p_u['id'].' LIMIT 1');
...
if(!$b_s){
$mysqli->query('INSERT INTO `boevaya_sila` SET `user` = '.$user['id'].', `local` = "2", `bon_col` = "1", `tip` = "2" ');
}else{
$mysqli->query("UPDATE `boevaya_sila` SET `local` = '2', `bon_col` = '1', `tip` = '2' WHERE `id` = '".$b_s['id']."' LIMIT 1");
}
if($p_u['id']){
$res = $mysqli->query('SELECT * FROM `users_tanks` WHERE `user` = '.$user['id'].' and `active`  = "1" LIMIT 1');
$u_t = $res->fetch_assoc();
...
if(!$b_s){
$mysqli->query('INSERT INTO `boevaya_sila` SET `user` = '.$user['id'].', `local` = "2", `bon_col` = "1", `tip` = "3" ');
}else{
$mysqli->query("UPDATE `boevaya_sila` SET `local` = '2', `bon_col` = '1', `tip` = '3' WHERE `id` = '".$b_s['id']."' LIMIT 1");
}
if($p_u['id']){
$res = $mysqli->query('SELECT * FROM `users_tanks` WHERE `user` = '.$user['id'].' and `active`  = "1" LIMIT 1');
$u_t = $res->fetch_assoc();
...
$res = $mysqli->query('SELECT * FROM `pve` WHERE `time_pve` > "'.time().'"  LIMIT 1');
$p1 = $res->fetch_assoc();
if($p1){
$res = $mysqli->query('SELECT * FROM `pve` WHERE `id` and `time_pve` = "0" ORDER BY `time` asc LIMIT 7');
}elsE{
$res = $mysqli->query('SELECT * FROM `pve` WHERE `id` ORDER BY `time` asc LIMIT 1,7');
}
while ($p = $res->fetch_array()){
if($p['tip']==1){$tipB = 'Оборонительная операция';}else{$tipB = 'Наступательная операция';}
echo '<div class="mb5 inbl"><div class="thumb fl"><img src="/images/battles/batt'.$p['id'].'.jpg" style="width:100%; border-radius: 9px;"><span class="mask2">&nbsp;</span></div><div class="ml58 small white sh_b bold">
<span class="green2">'.$p['name'].'</span><br>
<span>
<span>до начала '._time1($p['time']-time()).'</span>
</span><br>
<span class="gray1">'.$tipB.'</span>
...
echo '<div class="bot"><a w:id="past" class="simple-but gray border" href="/pve/pvepast/"><span><span>Прошедшие сражения</span></span></a></div>';
...
$mysqli->query('DELETE FROM `pve_results` WHERE `time` < "'.(time()-(3600*20)).'" ');
